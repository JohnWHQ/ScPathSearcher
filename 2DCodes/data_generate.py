...
class DataGenerator:
			...
			for j in range(self.col):
				...
				if self.matrix[i][j] == DataGenerator._sp_point:
					...
					if cur_rand >= 1 and cur_rand <= 2:
						self._generate_uni_shape(i, j, 208, 147 ,54, 104, direction_rand)

					# 3% probability to generate 188 (133)
					if cur_rand >= 3 and cur_rand <= 5:
						self._generate_uni_shape(i, j, 188, 133 ,48, 94, direction_rand)

					# 7% probability to generate 146 (103)
					if cur_rand >= 6 and cur_rand <= 12:
						self._generate_uni_shape(i, j, 146, 103 ,38, 73, direction_rand)

					# 10% probability to generate 125 (88)
					if cur_rand >= 13 and cur_rand <= 22:
						self._generate_uni_shape(i, j, 125, 88 ,32, 63, direction_rand)

					# 16% probability to generate 104 (74)
					if cur_rand >= 23 and cur_rand <= 38:
						self._generate_uni_shape(i, j, 104, 74 ,27, 52, direction_rand)

					# 17% probability to generate 63 (45)
					if cur_rand >= 39 and cur_rand <= 55:
						self._generate_uni_shape(i, j, 64, 45 ,16, 32, direction_rand)

					# 30% probability to generate 42 (30)
					if cur_rand >= 56 and cur_rand <= 85:
						self._generate_uni_shape(i, j, 42, 30 ,11, 21, direction_rand)

					# 15% probability to generate 21 (15)
					if cur_rand >= 86 and cur_rand <= 100:
						self._generate_uni_shape(i, j, 21, 15 ,5, 11, direction_rand)
	...
	def _generate_uni_shape(self, x, y, length0, width45, h15, h30, percent_rand):
		"""
		...
		if percent_rand > 50 and percent_rand <= 75:
			...
			if (h15 & 1) == 1:
				...
				self._generate_sub_shape_from_mid(x, y, 2)
			...
			for i in range(1, (cur_half + 1)):
				if (i % 3) == 0:
					self._generate_sub_shape_from_left_2_right(dx_right_up, dy_right_up, 3)
					self._generate_sub_shape_from_right_2_left(dx_left_down, dy_left_down, 3)
					...
					self._generate_sub_shape_from_left_2_right(dx_right_up, dy_right_up, 4)
					self._generate_sub_shape_from_right_2_left(dx_left_down, dy_left_down, 4)
		...
		if percent_rand > 75 and percent_rand <= 100:
			...
			if (h30 & 1) == 1:
				...
				self._generate_sub_shape_from_mid(x, y, 1)
			...
			for i in range(1, (cur_half + 1)):
				if (i % 3) == 0:
					self._generate_sub_shape_from_left_2_right(dx_right_up, dy_right_up, 1)
					self._generate_sub_shape_from_right_2_left(dx_left_down, dy_left_down, 1)
					...
					self._generate_sub_shape_from_left_2_right(dx_right_up, dy_right_up, 2)
					self._generate_sub_shape_from_right_2_left(dx_left_down, dy_left_down, 2)
	...
	def _generate_sub_shape_from_mid(self, x, y, diff):
		# only for odd symmetry
		if (x < 0) or (x >= self.row) or (y < 0) or (y >= self.col): 
			return

		self.matrix[x][y] = DataGenerator._selected

		for i in range(1, diff):
			cur_right_y =  y + i
			cur_left_y = y - i
			if cur_right_y < self.col:
				self.matrix[x][cur_right_y] = DataGenerator._selected
			if cur_left_y >= 0:
				self.matrix[x][cur_left_y] = DataGenerator._selected			


	def _generate_sub_shape_from_left_2_right(self, x, y, diff):
		# include (x, y)
		if (x < 0) or (x >= self.row) or (y < 0) or (y >= self.col): 
			return
		for i in range(diff):
			dy = y + i
			if dy < self.col:
				self.matrix[x][dy] = DataGenerator._selected


	def _generate_sub_shape_from_right_2_left(self, x, y, diff):
		# include (x, y)
		if (x < 0) or (x >= self.row) or (y < 0) or (y >= self.col): 
			return
		for i in range(diff):
